...
async def handle_instagram_link(message: Message):
    ...
    try:
        ...
        if media_type == 'carousel' and len(items) > 1:
            media_group = []
            for idx, item in enumerate(items[:10]): # Telegram max 10 media in media group
                fp = item['filepath']
            ...
            if media_group:
                ...
                await status_msg.edit_text("⚠️ <b>Fayl hajmi 50 MB dan katta bo'lgani uchun Telegram orqali yuborib bo'lmadi.</b>", parse_mode="HTML")
            ...
            if not is_valid:
                await status_msg.edit_text(f"⚠️ <b>Video hajmi juda katta ({size_mb} MB). Telegram botlar maksimal 50 MB yuklay oladi.</b>", parse_mode="HTML")
        ...
        err_msg = clean_html(str(e))
        await status_msg.edit_text(f"❌ <b>Xatolik yuz berdi:</b> {err_msg}\n\nIltimos, havolani qayta tekshirib ko'ring.", parse_mode="HTML")
...
async def cb_extract_mp3(callback: CallbackQuery):
    ...
    try:
        ...
        await status_msg.edit_text(f"❌ Musiqani ajratishda xatolik: {clean_html(str(e))}")
...
async def cb_make_round_inline(callback: CallbackQuery):
    ...
    status_msg = await msg.reply("⭕ <b>Videoni dumaloq shaklga keltirish qilinmoqda...</b>", parse_mode="HTML")
    ...
    try:
        ...
        await status_msg.edit_text(f"❌ Dumaloq video yaratishda xatolik: {clean_html(str(e))}")
        safe_remove_files(download_path)
...
async def cb_speed_video(callback: CallbackQuery):
    ...
    status_msg = await msg.reply("⚡ <b>Video 1.5x tezlashtirilmoqda...</b>", parse_mode="HTML")
    ...
    try:
        ...
        await msg.reply_video(video=fast_video, caption="⚡ <b>1.5x Tezlashtirilgan Video</b>\n\n🤖 @InstaOhang_bot", parse_mode="HTML")
        await status_msg.delete()
        safe_remove_files(download_path, fast_path)
    except Exception as e:
