...
async def process_music_search(message: Message, query: str):
    ...
    status_msg = await message.answer(f"🔍 <b>'{clean_q}' bo'yicha musiqalar qidirilmoqda...</b>", parse_mode="HTML")
    
    try:
        results = await search_music_results(query, max_results=10)
        if not results:
            await status_msg.edit_text(f"❌ <b>'{clean_q}'</b> bo'yicha musiqa topilmadi. Qayta urinib ko'ring!", parse_mode="HTML")
            return
        ...
        for idx, item in enumerate(results, start=1):
            ...
            text_lines.append(f"<b>{idx}.</b> 🎵 {display_title} <i>({duration})</i>")
            
            btn_text = f"{idx}. 🎵 {display_title}"
            inline_buttons.append([InlineKeyboardButton(text=btn_text, callback_data=f"dl_music:{item['id']}")])
        ...
        await status_msg.edit_text(f"❌ Musiqa qidirishda xatolik: {clean_html(str(e))}")
...
async def cb_download_music(callback: CallbackQuery):
    ...
    if not is_subbed:
        await callback.answer("🔒 Botdan foydalanish uchun kanallarga obuna bo'ling!", show_alert=True)
        return
    ...
    try:
        ...
        if not is_valid:
            await status_msg.edit_text(f"⚠️ <b>Audio hajmi juda katta ({size_mb} MB). Telegram botlar maksimal 50 MB yuklay oladi.</b>", parse_mode="HTML")
        ...
        await status_msg.edit_text(f"❌ Musiqani yuklashda xatolik: {clean_html(str(e))}")
...
async def cmd_music_search(message: Message):
    ...
    if not query:
        await message.answer("⚠️ Iltimos, qo'shiq nomini yozing!\n<i>Misol:</i> <code>Konsta</code>", parse_mode="HTML")
        return
...
async def cmd_audio_extract(message: Message):
    ...
    if not target_msg.video:
        await message.answer("⚠️ Musiqani ajratish uchun videoga javoban <code>/audio</code> deb yozing!", parse_mode="HTML")
        return
    ...
    try:
        ...
        is_valid, size_mb = check_file_size(mp3_path)
        if not is_valid:
            await status_msg.edit_text(f"⚠️ <b>Audio hajmi juda katta ({size_mb} MB).</b>", parse_mode="HTML")
            safe_remove_files(temp_video, mp3_path)
            return

        audio_file = FSInputFile(mp3_path)
        await message.answer_audio(audio=audio_file, caption="🎵 <b>MP3 Audio</b>\n🤖 @InstaOhang_bot", parse_mode="HTML")
        await status_msg.delete()
        safe_remove_files(temp_video, mp3_path)
    except Exception as e:
        logger.error(f"Cmd audio extract error: {e}")
        await status_msg.edit_text(f"❌ Audio ajratishda xatolik: {clean_html(str(e))}")
...
async def cmd_video_fast(message: Message):
    target_msg = message.reply_to_message if message.reply_to_message else message
    if not target_msg.video:
        await message.answer("⚠️ Videoga reply qilib <code>/fast</code> deb yozing!", parse_mode="HTML")
        return
    ...
    try:
        ...
        if not is_valid:
            await status_msg.edit_text(f"⚠️ <b>Video hajmi juda katta ({size_mb} MB).</b>", parse_mode="HTML")
            safe_remove_files(temp_video, fast_path)
            return

        video_file = FSInputFile(fast_path)
        await message.answer_video(video=video_file, caption="⚡ <b>1.5x Tezlashtirilgan Video</b>", parse_mode="HTML")
        await status_msg.delete()
        safe_remove_files(temp_video, fast_path)
    except Exception as e:
        logger.error(f"Cmd fast error: {e}")
        await status_msg.edit_text(f"❌ Video tezlashtirishda xatolik: {clean_html(str(e))}")
        safe_remove_files(temp_video)
...
async def cmd_video_slow(message: Message):
    target_msg = message.reply_to_message if message.reply_to_message else message
    if not target_msg.video:
        await message.answer("⚠️ Videoga reply qilib <code>/slow</code> deb yozing!", parse_mode="HTML")
        return
    ...
    try:
        ...
        await message.answer_video(video=video_file, caption="🐢 <b>0.8x Sekinlashtirilgan Video</b>", parse_mode="HTML")
        await status_msg.delete()
        safe_remove_files(temp_video, slow_path)
    except Exception as e:
        logger.error(f"Cmd slow error: {e}")
        await status_msg.edit_text(f"❌ Video sekinlashtirishda xatolik: {clean_html(str(e))}")
        safe_remove_files(temp_video)
